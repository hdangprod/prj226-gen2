...
### GOV-018 — Engineering Phase authorization

- **Question:** Authorize a separately scoped PRJ226 Generation 2 Engineering Phase to implement the approved Liam v1 Product Foundation and Runtime Architecture under the approved Delivery Contract.
- **Status:** APPROVED
- **Decision owner:** `github:hdangprod`
- **Rationale:** The Generation 2 Foundation Program is complete through `GOV-015`, the Delivery Contract revision 1 is approved through `GOV-013`, and the Runtime Architecture revision 1 is approved through `GOV-017`. The human project owner has explicitly authorized post-Foundation, post-Architecture implementation within those approved boundaries.
- **Alternatives:** Keep engineering unauthorized; authorize bounded engineering implementation under the approved baselines; authorize broader product, architecture, production, paid-service, or control-plane work.
- **Normative dependencies:** `GOV-015`; `GOV-011`; `GOV-013`; `GOV-017`; `product/PRODUCT_FOUNDATION.md` revision 1; `development/DELIVERY_CONTRACT.md` revision 1; `docs/architecture/RUNTIME_ARCHITECTURE.md` revision 1.
- **Affected artifacts:** `docs/foundation/DECISIONS.md`, `docs/development/CURRENT.md`, `docs/README.md`, authorized Engineering Plan and Task Packets, and later implementation artifacts and Delivery Records created by valid Ready tasks.
- **Approved decision revision:** 1.
- **Approved on:** 2026-08-09.
- **Approved by:** `github:hdangprod`.
- **Approval evidence:** The human project owner explicitly authorized the separately scoped “PRJ226 Generation 2 Engineering Phase,” directed that the authorization be recorded durably before Engineering work proceeds, and authorized implementation of the currently approved Liam v1 Product Foundation and Runtime Architecture under Delivery Contract revision 1.
- **Authorized inputs:** Approved repository governance; Product Foundation revision 1 and its approved Product Intent, Scenario Corpus, Domain Model, and decision dependencies; Delivery Contract revision 1; Runtime Architecture revision 1 and its approved `ARC-001` through `ARC-006` dependencies; C7 Engineering-Entry Readiness Evidence revision 1 as accepted derived evidence; and the current operational state.
- **Authorized scope:** Runtime and TypeScript application code; the single Cloudflare Workers deployable; D1 schema, migrations, persistence port and adapter; Project, Action, Knowledge Item, accepted progress/context, provenance, supersession, deletion, export, and direct-SQL retrieval behavior; the Liam-owned Model Capability Port and Workers AI adapter; `@cf/zai-org/glm-4.7-flash` as the initial free-profile validation candidate; interaction orchestration and human-control enforcement; data-minimized runtime observability; failure and recovery behavior; deterministic, integration, contract, and semantic scenario tests; development and local/test configuration; bounded Task Packets, Delivery Records, and necessary engineering documentation.
- **Engineering execution contract:** Every implementation assignment requires a valid Task Packet satisfying `development/DELIVERY_CONTRACT.md` revision 1. A task may narrow this authorization but cannot broaden it. Accepted-state success requires required authoritative persistence success. The application/domain boundary remains authoritative; D1 and model/provider representations remain adapter-local; direct authoritative SQL retrieval is the initial retrieval mechanism; no provider-hosted state is canonical memory.
- **Cost and provider boundary:** Engineering targets the approved approximately-zero-cost Cloudflare profile. Free-tier exhaustion is an explicit operational failure or Human Reserved upgrade decision, not product truth. No paid dependency, paid provider use, billing change, or cost-incurring production resource is authorized. A demonstrated free-profile capability gap must be recorded with evidence, alternatives, expected cost, architecture impact, and recommendation before human disposition.
- **Does not authorize:** Any change to approved Product Foundation semantics or `ARC-001` through `ARC-006`; a new architecture boundary or additional runtime service; vector, embedding, search, cache, or queue infrastructure contrary to the approved architecture; weakened human-control, deletion, or data-control semantics; paid-service activation or billing changes; production deployment or provisioning; production credentials or secrets; destructive production operations; or implementation of a development control plane, scheduler, agent runner, automatic router, orchestration database, or `.dev-control` system.
- **Human Reserved boundary:** Product or architecture semantic changes, security-boundary decisions, paid usage or billing, production deployment or destructive production action, unresolved authoritative conflict, unapproved scope expansion, and every other repository-reserved decision require explicit human disposition. A discovered contradiction is recorded in a structured Decision Packet rather than resolved by implementation convenience.
- **Planning-session boundary:** The authorization permits later execution of Ready Engineering Task Packets. The authorizing planning session is limited to governance recording, readiness assessment, implementation planning, Task Packet preparation, and verification of those planning artifacts; it does not dispatch routine Builder execution.
- **Editorial-only change attestation:** Adding the Engineering Plan ownership-map row and updating the live Current Project State record the new authorized operational artifacts and state. They do not change documentation-control semantics, Foundation gate meanings, Product Foundation semantics, Delivery Contract semantics, or Runtime Architecture semantics.
- **Supersedes:** None.
- **Superseded by:** None.
