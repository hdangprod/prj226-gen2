...
| [`product/PRODUCT_FOUNDATION.md`](../product/PRODUCT_FOUNDATION.md) | CANONICAL / APPROVED, revision 1 | Approved reconciliation of the Liam v1 product foundation and its integrated traceability | Created under human-approved C5 authorization `GOV-010`; approved through human G5 disposition `GOV-011`. |
| [`development/DELIVERY_CONTRACT.md`](../development/DELIVERY_CONTRACT.md) | CANONICAL / APPROVED, revision 1 | Approved provider-neutral delivery governance, Task Packet and Delivery Record contract, and engineering DoR/DoD | Created under human-approved C6 authorization `GOV-012`, with approved `DLV-001` and `DLV-002`; approved through human G6 disposition `GOV-013`. |
| [`architecture/ARCHITECTURE_PHASE.md`](architecture/ARCHITECTURE_PHASE.md) | OPERATIONAL / ACTIVE | Current Runtime Architecture Phase readiness, derived drivers, decision dependencies, and Human Reserved Decision Packets | Created under post-Foundation authorization `GOV-016`; it is not an architecture baseline and cannot approve an `ARC-*` decision. |
| [`architecture/RUNTIME_ARCHITECTURE.md`](architecture/RUNTIME_ARCHITECTURE.md) | CANONICAL / APPROVED, revision 1 | Approved Liam v1 runtime architecture, its responsibility boundaries, technology baseline, and architecture-level verification requirements | Created under post-Foundation authorization `GOV-016`; approved through human disposition `GOV-017` and cannot authorize engineering. |
| [`development/ENGINEERING_PLAN.md`](development/ENGINEERING_PLAN.md) | OPERATIONAL / ACTIVE | Current Engineering objective, readiness result, implementation DAG, task state, first execution wave, validation strategy, and Human Reserved boundaries | Created under human Engineering authorization `GOV-018`; it reflects approved authority and Task Packet state but cannot expand authority or establish implementation completion. |
