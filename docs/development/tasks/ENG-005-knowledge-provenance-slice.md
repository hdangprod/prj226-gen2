...
**Current task state:** `READY — BOUNDED_REPAIR_AUTHORIZED` — reviewed candidate `239535a075b7b10626fc1f1adf9687daf08b0088` failed fresh full independent review on `ENG-005-REV2-IR-F001`. A bounded in-lock repair and new candidate are required; ENG-005 is not `DONE`.
...
**Task-level DoR:** `READY — BOUNDED_REPAIR_AUTHORIZED`; the reviewed candidate `239535a075b7b10626fc1f1adf9687daf08b0088`, tree `d669c213f173711b5f064897d016b9189ac0ce7a`, and canonical seven-file aggregate `939cdae244aa44252ed6d12031874d406e3b5009955e3fc9e34f339140eed15a` are failed-review provenance only. The next Builder starts from that exact candidate and may repair only the finite Bearer discriminator and required task-scoped evidence inside the three exclusive roots. It must produce a new commit, tree, repair-delta identity, seven-file manifest, and canonical full aggregate. No lock expansion, target-selection API, clarification-result surface, generalized DLP/token-vocabulary classifier, historical-identity claim, or other semantic change is authorized. The replacement candidate requires fresh **full** deterministic verification under the whole contract and, if that passes, fresh **full** independent semantic/data and persistence-boundary review before Controller closure.
...
`ENG-005-REV2-IR-F001` is **BLOCKING — WORK_PRODUCT_DEFECT** for reviewed candidate `239535a075b7b10626fc1f1adf9687daf08b0088`. Its finite guard rejects `Authorization: Bearer tokenonly123` but accepts bare `Bearer tokenonly123` because that branch requires punctuation in the token. The bypass can reach both capture and correction persistence paths, violating invariant 11, Verification Contract check 8, `DATA-001`, and `PI-DATA-005`. It is distinct from `ENG-005-REV2-F002`: F002 corrected a false positive for benign Bearer prose, while IR-F001 is a newly exposed false-negative bypass for bare opaque Bearer material. The bounded repair is authorized under `GOV-018` with no Human Reserved decision: it must preserve the reconciled exact-target/retry boundary, alter no persistence/API/architecture boundary, and remain finite and service-local. Any newly exposed materially distinct production semantic defect stops work as `BLOCKED — ADDITIONAL_PRODUCTION_DEFECT_DISCOVERED` and returns to the Controller.
