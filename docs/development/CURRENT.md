...
**Engineering Definition of Ready:** PASS for bounded implementation planning. Accepted predecessor identities and closure evidence remain unchanged. `ENG-010` is `DONE / ACCEPTED / CANONICALIZED / POST-INTEGRATION VERIFIED / GOVERNANCE-CLOSED`. ENG-011 task-level Formal DoR revision 1 is `PASS`; READY does not dispatch implementation. No push exists.
...
**Next executable work:** `ENG-011 CONTROLLER — DURABLE READY / BUILDER DISPATCH`. Do not implement before that separate Controller role records the dispatch.
