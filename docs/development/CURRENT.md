...
- **Engineering Phase:** AUTHORIZED BY HUMAN through `GOV-018`; Engineering delivery is active with `ENG-001`, `ENG-002`, `ENG-003` revision 2, `ENG-004`, and `ENG-008` complete; `ENG-005` is Ready for an explicitly authorized rebuild; and no task Running.
...
**Current stage state:** Generation 2 Foundation Program — COMPLETE; Runtime Architecture Phase — COMPLETE; Engineering Phase — AUTHORIZED; Engineering planning — ACTIVE; Builder execution — `ENG-001`, `ENG-002`, `ENG-003` revision 2, `ENG-004`, and `ENG-008` COMPLETE; `ENG-005` READY — REBUILD_AUTHORIZED; no task RUNNING
...
**Next executable work:** [`ENG-005`](tasks/ENG-005-knowledge-provenance-slice.md) revision 2 may be dispatched for an explicit in-lock rebuild against the current accepted ENG-003/ENG-004 baseline. It must not claim historical-byte recovery; it requires fresh full deterministic verification and fresh full independent review before Controller closure. `ENG-009` still needs a separate planning pass.
