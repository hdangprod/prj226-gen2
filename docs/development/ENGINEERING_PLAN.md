...
ENG-001 Runtime/tooling foundation
  -> ENG-002 Domain/application kernel
       -> ENG-003 D1 authority foundation (DONE on accepted revision-2 lifecycle-transition baseline)
            ...
            -> ENG-005 Knowledge/provenance slice (READY: BOUNDED REPAIR) ------------+-> ENG-006 Direct-SQL retrieval/resumption
...
No task is `RUNNING`. `ENG-001`, `ENG-002`, `ENG-003`, `ENG-004`, and `ENG-008` are `DONE`. ENG-003 revision 2 is accepted on aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 aggregate remains historical accepted evidence. ENG-004 is accepted on aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2` after controlled path-scoped manifestation, final deterministic PASS, and full review GREEN; its F001/F002/F003/F003-R1 history remains durable. `ENG-005` is `READY — BOUNDED_REPAIR_AUTHORIZED`: candidate `239535a075b7b10626fc1f1adf9687daf08b0088` passed deterministic verification but failed full independent review on the blocking bare-Bearer bypass `ENG-005-REV2-IR-F001`. The repair must stay inside the existing task lock and produce new bytes before fresh full verification and review; exact-target ambiguity remains upstream `ENG-010` work. `ENG-009` remains `PROPOSED` because it has no Task Packet or task-level Definition of Ready re-evaluation. All other nodes remain `PROPOSED` pending their listed predecessors and a complete Ready Task Packet.
...
| `ENG-005` | Deliver intentional Knowledge capture, origin provenance, correction, supersession chains, and current standing. Depends on `ENG-002`, accepted repaired `ENG-003`. | `READY — BOUNDED_REPAIR_AUTHORIZED` — failed-review candidate `239535a075b7b10626fc1f1adf9687daf08b0088` has blocking bare-Bearer bypass `ENG-005-REV2-IR-F001`; repair only the finite in-lock discriminator and evidence, then require fresh full gates | HIGH data-semantic risk; fresh independent semantic/data review required | Origin immutability, atomic correction, chain/currentness, unrelated-item isolation, no casual capture, malformed/missing exact-target no-write, authentication-material exclusion, failure/duplicate tests |
...
| [`ENG-005 — Knowledge/provenance slice`](tasks/ENG-005-knowledge-provenance-slice.md) | 2 | `READY — BOUNDED_REPAIR_AUTHORIZED` | Candidate `239535a075b7b10626fc1f1adf9687daf08b0088` failed full independent review on blocking bare-Bearer bypass `ENG-005-REV2-IR-F001`; an in-lock repair then fresh full deterministic verification and full review are required. |
...
7. F003 is both an upstream accepted-artifact defect and an omitted persistence capability. The Controller reopened the existing boundary owner, ENG-003, under Task Packet revision 2 rather than creating a new task ID. Revision 2 is accepted on aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`. The historical ENG-004 work product was recovered exactly after its absence blocked initial bounded-repair dispatch; the final F003-R1 candidate is now accepted and manifested on aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2`. ENG-005 exact-byte recovery then failed across Git and bounded local recovery surfaces; Task Packet revision 2 authorizes a new in-lock rebuild under `GOV-018`, not a recovered candidate or semantic repair. This update dispatches no Builder.

The initial `ENG-001` and `ENG-002` wave was deliberately serial while implementation and test conventions were established. ENG-004 is now complete. ENG-005 may proceed through its explicit in-lock rebuild only; its historical candidate must not be regenerated or labeled recovered. ENG-009 still requires a separate Task Packet and task-level DoR.

## ENG-003 repair and downstream lock disposition

| Task | Exclusive source lock | Exclusive test locks |
| --- | --- | --- |
| `ENG-003` revision 2 | `src/application/ports/persistence/**`; `src/infrastructure/d1/**` | `tests/application/ports/persistence/**`; `tests/infrastructure/d1/**` |
| `ENG-004` downstream repair | `src/application/services/projectActionContext/**` | `tests/application/services/projectActionContext/**`; `tests/integration/d1/projectActionContext/**` |

The ENG-003 writer and ENG-004 writer are serialized. ENG-004 never receives persistence-path ownership. Migrations, root configuration, dependency files, domain/contracts, and all other paths remain protected for the upstream repair.
...
`ENG-005: READY — BOUNDED_REPAIR_AUTHORIZED — IR-F001 BARE-BEARER REPAIR ONLY`
...
- ENG-003 revision-2 persistence writing and ENG-004 closure are complete. ENG-005 historical bytes are irrecoverable; reviewed candidate `239535a075b7b10626fc1f1adf9687daf08b0088` failed independent review on a bare-Bearer bypass and is provenance only. Only its bounded in-lock repair is ready; the replacement requires fresh full verification and review. Ambiguous-target clarification remains an upstream ENG-010 responsibility.
...
- **Current task state:** no task is `RUNNING`. `ENG-003` revision 2 is `DONE` on accepted aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 exact candidate and closed finding history remain durable. `ENG-004` is `DONE` on accepted aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2`, with F001/F002/F003/F003-R1 closed and recovery provenance retained. `ENG-005` is `READY — BOUNDED_REPAIR_AUTHORIZED` under Task Packet revision 2 after full independent review rejected candidate `239535a075b7b10626fc1f1adf9687daf08b0088` for blocking bare-Bearer bypass `ENG-005-REV2-IR-F001`; the in-lock repair must produce new bytes, then pass fresh full verification and review. `ENG-009` remains `PROPOSED` until its separate Task Packet and DoR exist. Later tasks retain their declared dependencies.
