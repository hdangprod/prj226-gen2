...
| [`ENG-004 — Project/Action/context slice`](tasks/ENG-004-project-action-context-slice.md) | 2 | `READY_FOR_BOUNDED_REPAIR` | Start from the exact recovered historical seven-file baseline composed over ENG-003 rev2, then dispatch only the existing three-root bounded repair and require fresh verification plus fresh full review. |
