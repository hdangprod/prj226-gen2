...
| `ENG-009` | Implement the adapter-local Workers AI integration for the replaceable canonical candidate through an exact offline adapter contract, without live qualification. Depends on `ENG-001`, `ENG-008`. | `PROPOSED / NOT DISPATCHED` — predecessors DONE; Planning Revision 1 failed Formal DoR; [Planning Revision 2](tasks/ENG-009-workers-ai-adapter.md) prepared for a fresh Formal DoR rerun; `READY: NO`; Builder `NONE` | HIGH provider/data/untrusted-output risk; Builder-independent deterministic verification and Builder-independent semantic review required | Exact mocked binding contract and schema; single-read/TOCTOU evidence; failure matrix; task-local adapter suite; complete current regression matrix; adapter-local type/import/credential/diagnostic scans |
...
| [`ENG-009 — Workers AI Adapter`](tasks/ENG-009-workers-ai-adapter.md) | 2 | `PROPOSED / NOT DISPATCHED` | Revision 1 failed Formal DoR and is historical/non-operative. Revision 2 incorporates `ENG-009-DOR-R001` through `R008` for an independent Formal DoR rerun. Formal DoR has not yet run against Revision 2; `READY: NO`; Builder `NONE`. |
