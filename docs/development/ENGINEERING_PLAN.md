...
| `ENG-009` | Implement the adapter-local Workers AI integration for the replaceable canonical candidate through an exact offline adapter contract, without live qualification. Depends on `ENG-001`, `ENG-008`. | `READY / NOT DISPATCHED` — predecessors DONE; Revision 1 `FROZEN / UNACCEPTED / FAILED FORMAL DoR / HISTORICAL`; [Planning Revision 2](tasks/ENG-009-workers-ai-adapter.md) is `DoR-QUALIFIED / ACCEPTED FOR READY AUTHORITY`; independent [Formal DoR Revision 2](analysis/ENG-009_FORMAL_DoR_REV2_2026-08-24.md) `PASS`; `READY: YES`; Builder `NONE`; implementation `NOT STARTED`; not yet canonicalized | HIGH provider/data/untrusted-output risk; Builder-independent deterministic verification and Builder-independent semantic review required | Exact mocked binding contract and schema; single-read/TOCTOU evidence; failure matrix; task-local adapter suite; complete current regression matrix; adapter-local type/import/credential/diagnostic scans |
...
| [`ENG-009 — Workers AI Adapter`](tasks/ENG-009-workers-ai-adapter.md) | 2 | `READY / NOT DISPATCHED` | Revision 1 is `FROZEN / UNACCEPTED / FAILED FORMAL DoR / HISTORICAL`. Revision 2 is `DoR-QUALIFIED / ACCEPTED FOR READY AUTHORITY`; independent [Formal DoR Revision 2](analysis/ENG-009_FORMAL_DoR_REV2_2026-08-24.md) is `PASS`, closing `ENG-009-DOR-R001` through `R008`. `READY: YES`; Builder `NONE`; dispatch `NOT PERFORMED`; implementation `NOT STARTED`; not yet canonicalized. |
...
- `ENG-006` Repair Rev1 is accepted and `ENG-006` is `DONE`; no ENG-006 Builder authority remains active. `ENG-007` remains `PROPOSED` and requires its own complete Ready Task Packet before dispatch.
