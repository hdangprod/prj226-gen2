...
ENG-001 Runtime/tooling foundation
  -> ENG-002 Domain/application kernel
       -> ENG-003 D1 authority foundation (DONE on accepted revision-2 lifecycle-transition baseline)
            ...
            -> ENG-005 Knowledge/provenance slice (DONE) ---------------------------+-> ENG-006 Direct-SQL retrieval/resumption
...
No task is `RUNNING`. `ENG-001`, `ENG-002`, `ENG-003`, `ENG-004`, `ENG-005`, and `ENG-008` are `DONE`. ENG-003 revision 2 is accepted on aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 aggregate remains historical accepted evidence. ENG-004 is accepted on aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2` after controlled path-scoped manifestation, final deterministic PASS, and full review GREEN; its F001/F002/F003/F003-R1 history remains durable. `ENG-005` is `DONE` on final aggregate `333f27f33f5725751a3cb48bbd0009253faab26282e218883b6393c3d3ae90f0` after controlled path-scoped manifestation, final deterministic PASS, and full review GREEN; all findings (F001, V001, F002, IR-F001..IR-F005) are closed. `ENG-006` is `READY / NOT DISPATCHED` with formal DoR `PASS` ([`ENG-006_FORMAL_DoR_2026-08-15.md`](analysis/ENG-006_FORMAL_DoR_2026-08-15.md)); no Builder currently running; implementation not started; eligible for one bounded Builder dispatch. `ENG-009` remains `PROPOSED` pending its separate Task Packet and task-level DoR evaluation. All other nodes remain `PROPOSED` pending their listed predecessors and a complete Ready Task Packet.
...
| `ENG-006` | Deliver simplest-sufficient direct-SQL retrieval and accepted-context resumption with provenance, currentness, bounded cross-Project reuse, and uncertainty. Depends on `ENG-003`, `ENG-004`, `ENG-005`. | `READY / NOT DISPATCHED` — formal DoR `PASS` | MODERATE-HIGH retrieval/architecture risk; independent semantic/architecture review required | Query fixtures; superseded/deleted exclusion; qualified historical result; origin preservation; transient-session-loss recovery; no vector/search/cache dependency scan |
...
Both tasks consume `src/domain/**`, `src/application/contracts/**`, the accepted ENG-003 revision-2 persistence repair, `migrations/0001_authoritative_state.sql`, existing regression tests/helpers, root configuration, dependency files, and `src/index.ts` as read-only accepted upstream state. Their source locks remain disjoint; `ENG-004` and `ENG-005` are complete; `ENG-006` is `READY / NOT DISPATCHED` with one bounded Builder dispatch authorized after READY governance commit; no Builder is currently running; and no `ENG-009` or later Builder dispatch is authorized without its own approved Ready Task Packet.
