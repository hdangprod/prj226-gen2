...
ENG-001 Runtime/tooling foundation
  -> ENG-002 Domain/application kernel
       -> ENG-003 D1 authority foundation (DONE on accepted revision-2 lifecycle-transition baseline)
            ...
            -> ENG-005 Knowledge/provenance slice (READY — REBUILD_AUTHORIZED) ------+-> ENG-006 Direct-SQL retrieval/resumption
...
No task is `RUNNING`. `ENG-001`, `ENG-002`, `ENG-003`, `ENG-004`, and `ENG-008` are `DONE`. ENG-003 revision 2 is accepted on aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 aggregate remains historical accepted evidence. ENG-004 is accepted on aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2` after controlled path-scoped manifestation, final deterministic PASS, and full review GREEN; its F001/F002/F003/F003-R1 history remains durable. `ENG-005` is `READY — REBUILD_AUTHORIZED` under Task Packet revision 2: exhaustive bounded recovery found none of the exact historical eight-file bytes, so reconciliation is impossible; `GOV-018` instead authorizes a fresh in-lock rebuild of already-approved semantics, followed by fresh full deterministic verification and fresh full independent review. `ENG-009` remains `PROPOSED` because it has no Task Packet or task-level Definition of Ready re-evaluation. All other nodes remain `PROPOSED` pending their listed predecessors and a complete Ready Task Packet.
...
| `ENG-005` | Deliver intentional Knowledge capture, origin provenance, correction, supersession chains, and current standing. Depends on `ENG-002`, accepted repaired `ENG-003`. | `READY — REBUILD_AUTHORIZED` — lost historical bytes cannot be reconciled; rebuild within revision-2 lock, then run fresh full deterministic verification and fresh full independent review | HIGH data-semantic risk; fresh independent semantic/data review required | Origin immutability, atomic correction, chain/currentness, unrelated-item isolation, no casual capture, authentication-material exclusion, failure/duplicate tests |
...
| [`ENG-005 — Knowledge/provenance slice`](tasks/ENG-005-knowledge-provenance-slice.md) | 2 | `READY — REBUILD_AUTHORIZED` | Exact-byte recovery failed; rebuild only the approved slice against current ENG-003/ENG-004 state, then run fresh full deterministic verification and fresh full independent review. |
...
`ENG-005: READY — REBUILD_AUTHORIZED — NO SEMANTIC REPAIR`
...
- ENG-003 revision-2 persistence writing and ENG-004 closure are complete. ENG-005 historical bytes are irrecoverable; only the explicitly authorized in-lock rebuild, fresh evidence, and fresh independent review may precede any downstream disposition.
...
- **Current task state:** no task is `RUNNING`. `ENG-003` revision 2 is `DONE` on accepted aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 exact candidate and closed finding history remain durable. `ENG-004` is `DONE` on accepted aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2`, with F001/F002/F003/F003-R1 closed and recovery provenance retained. `ENG-005` is `READY — REBUILD_AUTHORIZED` under Task Packet revision 2; its historical aggregate and F001 closure remain provenance, but no historical source byte was recovered. `ENG-009` remains `PROPOSED` until its separate Task Packet and DoR exist. Later tasks retain their declared dependencies.
