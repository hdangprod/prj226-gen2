...
ENG-001 Runtime/tooling foundation
  -> ENG-002 Domain/application kernel
       -> ENG-003 D1 authority foundation (Revision 3 accepted; DONE)
            ...
            -> ENG-005 Knowledge/provenance slice (DONE) ---------------------------+-> ENG-006 Direct-SQL retrieval/resumption (Repair unblocked at upstream capability level; not dispatched)
...
No task is `RUNNING`. `ENG-001`, `ENG-002`, `ENG-003`, `ENG-004`, `ENG-005`, and `ENG-008` are `DONE`. `ENG-003` revision 3 is `ACCEPTED` on commit `5276481824e43d23345799c39efaa72e51235877`, tree `0ea10d0400a5439af60c72ce943b7504e4173674`, and aggregate `a5bb90a62af050b2cc7bcf1beecac072b3927b45d91178e65564935d7420c156`; prior ENG-003 revisions remain historical accepted evidence. `ENG-003-R3-R001`, `ENG-003-R3-R002`, and `ENG-003-R3-S2-R001` are `CLOSED`; `GCV-001` is `CLOSED BY GOVERNANCE SUCCESSOR REPAIR`. Failed `ENG-006` candidate `7b7db0d98660f6562f8e9738445be725fc65988c` remains `FROZEN`; `ENG-006-R004` is `CLOSED AT UPSTREAM ENG-003 LEVEL`, while `ENG-006` is `REPAIR UNBLOCKED / NOT DISPATCHED / NOT ACCEPTED`; `R001`–`R003` are open for downstream repair and `R005` is deferred to `ENG-010`. No repair Builder is authorized; current Builder is `NONE`. `ENG-009` remains `PROPOSED / NOT DISPATCHED`. All other nodes remain `PROPOSED` pending their listed predecessors and a complete Ready Task Packet.
...
| `ENG-006` | Deliver simplest-sufficient direct-SQL retrieval and accepted-context resumption with provenance, currentness, bounded cross-Project reuse, and uncertainty. Depends on `ENG-003`, `ENG-004`, `ENG-005`. | `REPAIR UNBLOCKED / NOT DISPATCHED / NOT ACCEPTED` — Candidate `7b7db0d98660f6562f8e9738445be725fc65988c` FROZEN; R001–R003 open for downstream repair, R004 closed at upstream ENG-003 level, R005 deferred to ENG-010; no repair Builder authorized | MODERATE-HIGH retrieval/architecture risk; independent semantic/architecture review required | Query fixtures; superseded/deleted exclusion; qualified historical result; origin preservation; transient-session-loss recovery; no vector/search/cache dependency scan |
...
8. The initial `ENG-006` candidate (`7b7db0d98660f6562f8e9738445be725fc65988c`) failed semantic review. Finding `ENG-006-R004` identified an upstream D1 collection-read capability gap. `ENG-003` Revision 3 accepted the targeted two-file capability repair. `ENG-006-R004` is closed at upstream ENG-003 level; downstream `ENG-006` repair remains unaccepted and not dispatched.

The initial `ENG-001` and `ENG-002` wave was deliberately serial while implementation and test conventions were established. `ENG-004` and `ENG-005` are complete. `ENG-003` revision 3 is `ACCEPTED` and `ENG-003` is `DONE`; its Builder authority is consumed and non-operative. `ENG-006` repair is `UNBLOCKED AT UPSTREAM CAPABILITY LEVEL / NOT DISPATCHED / NOT ACCEPTED` (candidate `7b7db0d9...` FROZEN; `R001`–`R003` open, `R004` closed upstream, `R005` deferred to `ENG-010`). `ENG-009` remains `PROPOSED / NOT DISPATCHED`, requiring a separate Task Packet and task-level DoR.
...
Both tasks consume `src/domain/**`, `src/application/contracts/**`, the accepted ENG-003 persistence repair, `migrations/0001_authoritative_state.sql`, existing regression tests/helpers, root configuration, dependency files, and `src/index.ts` as read-only accepted upstream state. Their source locks remain disjoint; `ENG-004` and `ENG-005` are complete; `ENG-003` revision 3 is `ACCEPTED` and `ENG-003` is `DONE`; `ENG-006` repair is `UNBLOCKED AT UPSTREAM CAPABILITY LEVEL` but remains `NOT DISPATCHED` and `NOT ACCEPTED`; Current Builder is `NONE`; and no `ENG-009` or later Builder dispatch is authorized without its own approved Ready Task Packet.
...
`ENG-006: REPAIR UNBLOCKED AT UPSTREAM CAPABILITY LEVEL / NOT DISPATCHED / NOT ACCEPTED (CANDIDATE 7b7db0d9... FROZEN)`
...
- `ENG-006` has its accepted upstream capability baseline, but its downstream repair remains undispatched and unaccepted. `ENG-007` waits for retrieval and persistence so deletion/export evidence covers actual authority paths.
...
- **Current task state:** no task is `RUNNING`. `ENG-003` revision 3 is `ACCEPTED` and `ENG-003` is `DONE` on candidate `5276481824e43d23345799c39efaa72e51235877`, tree `0ea10d0400a5439af60c72ce943b7504e4173674`, and aggregate `a5bb90a62af050b2cc7bcf1beecac072b3927b45d91178e65564935d7420c156`; its prior accepted history remains durable. `ENG-004` and `ENG-005` remain `DONE`. Failed `ENG-006` candidate `7b7db0d98660f6562f8e9738445be725fc65988c` remains `FROZEN`; `ENG-006-R004` is `CLOSED AT UPSTREAM ENG-003 LEVEL`, while `ENG-006` repair is `UNBLOCKED AT UPSTREAM CAPABILITY LEVEL / NOT DISPATCHED / NOT ACCEPTED`; `R001`–`R003` remain open for downstream repair and `R005` is deferred to `ENG-010`. `ENG-009` remains `PROPOSED / NOT DISPATCHED` until its separate Task Packet and task-level DoR evaluation exist. Later tasks retain their declared dependencies.
