...
ENG-001 Runtime/tooling foundation
  -> ENG-002 Domain/application kernel
       -> ENG-003 D1 authority foundation (Revision 2 baseline accepted; Revision 3 targeted reopen READY / NOT DISPATCHED)
            -> ENG-004 Project/Action/context slice (DONE) ------------------------+
            -> ENG-005 Knowledge/provenance slice (DONE) ---------------------------+-> ENG-006 Direct-SQL retrieval/resumption (Repair BLOCKED on ENG-003 Rev3 acceptance)
...
No task is `RUNNING`. `ENG-001`, `ENG-002`, `ENG-003` revision 2, `ENG-004`, `ENG-005`, and `ENG-008` are `DONE`. ENG-003 revision 2 is accepted on aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 aggregate remains historical accepted evidence. `ENG-003` revision 3 targeted reopen is `READY / NOT DISPATCHED` with formal DoR `PASS` ([`ENG-003_REV3_FINAL_TARGETED_CONTROLLER_REVIEW_2026-08-15.md`](analysis/ENG-003_REV3_FINAL_TARGETED_CONTROLLER_REVIEW_2026-08-15.md)); one bounded Builder is authorized for dispatch after the governance promotion commit exists; no Builder is currently running. ENG-004 is accepted on aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2` after controlled path-scoped manifestation, final deterministic PASS, and full review GREEN; its F001/F002/F003/F003-R1 history remains durable. `ENG-005` is `DONE` on final aggregate `333f27f33f5725751a3cb48bbd0009253faab26282e218883b6393c3d3ae90f0` after controlled path-scoped manifestation, final deterministic PASS, and full review GREEN; all findings (F001, V001, F002, IR-F001..IR-F005) are closed. `ENG-006` candidate `7b7db0d98660f6562f8e9738445be725fc65988c` failed semantic review (findings `R001`–`R005`) and remains `FROZEN`; `ENG-006` repair is `BLOCKED ON ENG-003 REV3 ACCEPTANCE` ([`ENG-006_CONTROLLER_FINDING_DISPOSITION_2026-08-15.md`](analysis/ENG-006_CONTROLLER_FINDING_DISPOSITION_2026-08-15.md)); findings `R001`–`R003` are accepted for later repair, `R004` root blocker is addressed by `ENG-003` Rev3, and `R005` is deferred to `ENG-010`; no repair Builder is authorized pending upstream acceptance. `ENG-009` remains `PROPOSED` pending its separate Task Packet and task-level DoR evaluation. All other nodes remain `PROPOSED` pending their listed predecessors and a complete Ready Task Packet.
...
| `ENG-003` | Create and maintain the minimal D1 persistence port/adapter, constraints, atomic accepted writes and authoritative expected-state lifecycle transitions, retry/idempotency, and false-success defenses. Expose collection-read capability in revision 3. Depends on `ENG-002`. | `READY / NOT DISPATCHED` (Revision 3 targeted reopen; formal DoR `PASS`); revision-2 aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685` accepted historical baseline; revision-1 aggregate `e8f3792925ad45905a72938c6602f860df3ff6173325944e77f9ff2c8642caf6` preserved as history | HIGH persistence/lifecycle/data risk; fresh full independent persistence review GREEN for rev 2; independent review for rev 3 | Insert-only initial state; authoritative existence/ownership/expected-state transitions; concurrent/stale conflict; receipt atomicity; `37/37` local-D1 persistence evidence; [Delivery Record](delivery/ENG-003-d1-authority-foundation.md) |
...
| `ENG-006` | Deliver simplest-sufficient direct-SQL retrieval and accepted-context resumption with provenance, currentness, bounded cross-Project reuse, and uncertainty. Depends on `ENG-003`, `ENG-004`, `ENG-005`. | `REPAIR BLOCKED ON ENG-003 REV3 ACCEPTANCE` — Candidate `7b7db0d98660f6562f8e9738445be725fc65988c` FROZEN; R001–R003 accepted, R004 addressed via ENG-003 Rev3, R005 deferred to ENG-010; no repair Builder authorized pending upstream acceptance | MODERATE-HIGH retrieval/architecture risk; independent semantic/architecture review required | Query fixtures; superseded/deleted exclusion; qualified historical result; origin preservation; transient-session-loss recovery; no vector/search/cache dependency scan |
...
| [`ENG-003 — D1 Authority Foundation`](tasks/ENG-003-d1-authority-foundation.md) | 3 | `READY / NOT DISPATCHED` | Revision 2 accepted historical baseline; Revision 3 targeted reopen authorized for dispatch after governance promotion commit. |
...
8. The initial `ENG-006` candidate (`7b7db0d98660f6562f8e9738445be725fc65988c`) failed semantic review. Finding `ENG-006-R004` identified an upstream D1 collection-read capability gap. `ENG-003` is reopened under Revision 3 as a targeted two-file reopen (`READY / NOT DISPATCHED`). `ENG-006` repair remains `BLOCKED` on `ENG-003` Revision 3 acceptance.

The initial `ENG-001` and `ENG-002` wave was deliberately serial while implementation and test conventions were established. `ENG-004` and `ENG-005` are now complete. `ENG-003` revision 3 targeted reopen is `READY / NOT DISPATCHED` (formal DoR `PASS`; one bounded Builder authorized after governance promotion commit; no Builder currently running; implementation not started). `ENG-006` repair is `BLOCKED ON ENG-003 REV3 ACCEPTANCE` (candidate `7b7db0d9...` FROZEN; findings `R001`–`R003` accepted, `R004` addressed via `ENG-003` Rev3, `R005` deferred to `ENG-010`). `ENG-009` remains `PROPOSED`, requiring a separate Task Packet and task-level DoR.
...
| `ENG-003` revision 3 (Targeted Reopen) | `src/infrastructure/d1/d1Types.ts` | `tests/infrastructure/d1/fakeD1.ts` |
...
The ENG-003 Rev3 writer is strictly bounded to two files. All other paths (persistence implementations, migrations, root configuration, dependency files, domain/contracts, application services) remain protected.
...
Both tasks consume `src/domain/**`, `src/application/contracts/**`, the accepted ENG-003 revision-2 persistence repair, `migrations/0001_authoritative_state.sql`, existing regression tests/helpers, root configuration, dependency files, and `src/index.ts` as read-only accepted upstream state. Their source locks remain disjoint; `ENG-004` and `ENG-005` are complete; `ENG-003` revision 3 is `READY / NOT DISPATCHED` with one bounded Builder dispatch authorized after governance commit; `ENG-006` repair is `BLOCKED ON ENG-003 REV3 ACCEPTANCE`; no Builder is currently running; and no `ENG-009` or later Builder dispatch is authorized without its own approved Ready Task Packet.
...
`ENG-003 REVISION 3: READY / NOT DISPATCHED (FORMAL DoR PASS)`

`ENG-004: DONE — F001/F002/F003/F003-R1 CLOSED`

`ENG-005: DONE — F001/V001/F002/IR-F001..IR-F005 CLOSED`

`ENG-006: REPAIR BLOCKED ON ENG-003 REV3 ACCEPTANCE (CANDIDATE 7b7db0d9... FROZEN)`
...
- **Current task state:** no task is `RUNNING`. `ENG-003` revision 2 is `DONE` on accepted aggregate `183d97eeb8f1f1d9a718d40ceba03071c79432132ae9febeb851ed163301a685`; its revision-1 exact candidate and closed finding history remain durable. `ENG-003` revision 3 targeted reopen is `READY / NOT DISPATCHED` (formal DoR `PASS`; one bounded Builder authorized after governance promotion commit; no Builder currently running; implementation not started). `ENG-004` is `DONE` on accepted aggregate `6be8bc2b4d58cd1a0e9be7ea6a3762dafee0aa5a26796bb8e97214e48c1725c2`, with F001/F002/F003/F003-R1 closed and recovery provenance retained. `ENG-005` is `DONE` on accepted aggregate `333f27f33f5725751a3cb48bbd0009253faab26282e218883b6393c3d3ae90f0` after controlled manifestation, fresh deterministic PASS, and independent review GREEN closing findings `F001` through `IR-F005`. `ENG-006` candidate `7b7db0d98660f6562f8e9738445be725fc65988c` failed semantic review and remains `FROZEN`; `ENG-006` repair is `BLOCKED ON ENG-003 REV3 ACCEPTANCE` (findings `R001`–`R003` accepted, `R004` addressed via `ENG-003` Rev3, `R005` deferred to `ENG-010`). `ENG-009` remains `PROPOSED` until its separate Task Packet and task-level DoR evaluation exist. Later tasks retain their declared dependencies.
