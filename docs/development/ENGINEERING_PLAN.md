...
- `ENG-006` Repair Rev1 is accepted and `ENG-006` is `DONE`; no ENG-006 Builder authority remains active. `ENG-007` Planning Revision 1 and failed Planning Revision 2 candidate `2acc99a5e75ba58ea4df53d6705ae5d44fe5bad8` remain `FROZEN / UNACCEPTED / HISTORICAL`; Planning Revision 2 fresh repair successor is `PROPOSED / PREPARED FOR FORMAL DoR RECHECK`; Human Reserved decisions are resolved; Formal DoR is `NOT YET RUN`; Builder is `NONE`; implementation is `NOT STARTED`.
