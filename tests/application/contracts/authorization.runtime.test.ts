...
import { createHumanControlRuntime, type TrustedInteractionEvidence } from "../../../src/application/contracts/humanControl";
...
describe("runtime authorization enforcement", () => {
  it.each([undefined, {}, { kind: "ordinary-mutation-authorization" }])("rejects missing or fabricated authorization %#", (authorization) => {
    const application = createHumanControlRuntime();
    expect(createProject(projectId("p1"), text("Ship"), application.mutationGate, authorization as never)).toMatchObject({ kind: "authorization-rejected" });
  });

  it("exercises the trusted production happy path", () => {
    const application = createHumanControlRuntime();
    const evidence = application.trustedInteractionIngress.observeInteraction({ summary: "Create Project" });
    ...
    expect(createProject(projectId("p1"), text("Ship"), application.mutationGate, authorization)).toMatchObject({ kind: "valid-transition", value: { id: "p1" } });
  });
  ...
  it("rejects raw self-declaration, proposal, and fabricated evidence", () => {
    ...
    expect(createProject(projectId("p1"), text("Ship"), application.mutationGate, proposal as never)).toMatchObject({ kind: "authorization-rejected" });
  });

  it("rejects foreign-runtime capabilities at the application mutation gate", () => {
    const application = createHumanControlRuntime();
    const attacker = createHumanControlRuntime();
    const evidence = attacker.trustedInteractionIngress.observeInteraction({ summary: "Create Project" });
  ...
  it("rejects reuse across target, operation, content, and Knowledge", () => {
    ...
    const authorization = application.humanControl.authorizeOrdinaryChange(classification, scope);
    if (authorization.kind !== "ordinary-mutation-authorization") throw new Error("Trusted issuance failed");
    expect(createProject(projectId("p2"), text("Ship"), application.mutationGate, authorization)).toMatchObject({ kind: "authorization-rejected" });
    expect(createProject(projectId("p1"), text("Different"), application.mutationGate, authorization)).toMatchObject({ kind: "authorization-rejected" });
    expect(captureKnowledge({ items: [] }, { id: "k1" as never, originatingProjectId: projectId("p1"), content: text("Ship"), intentional: true }, application.mutationGate, authorization)).toMatchObject({ kind: "authorization-rejected" });
  });
});
